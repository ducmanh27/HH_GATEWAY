...
{
    initLogger("Application", "../log/applog_", false);
}
...
void Logger::initLogger(std::string appName, std::string prefixLogDir, bool isAsync)
{
    time_t rawtime;
    struct tm* timeinfo;
    char buffer[80];
    ...
    if (isAsync)
    {
        ...
        mAppLogger = std::make_shared<spdlog::async_logger>(APP_LOGGER_NAME,
                                                            begin(sinks),
                                                            end(sinks),
                                                            spdlog::thread_pool());
    }
    ...
    {
        mAppLogger = std::make_shared<spdlog::logger>(APP_LOGGER_NAME,
                                                      begin(sinks),
                                                      end(sinks));
    }
    ...
    mAppLogger->set_level(spdlog::level::trace);
    mAppLogger->flush_on(spdlog::level::debug);
    //[%Y-%m-%d %H:%M:%S.%e]: Day / Month / Year: Hour / Minute / Second : Miliseconds [000-999]
    //can use %f: Microseconds [000000-999999]
    //can use %F: Nanoseconds [000000000-999999999]
